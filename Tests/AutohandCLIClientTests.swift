...
@Suite(.serialized) struct AutohandCLIClientTests {
  ...
  @Test func autoresearchRequestTypesEncodeCLIValues() throws {
    ...
    @Test func autoModePauseUsesExactEmptyParametersAndDecodesResult() async throws {
      let fixture = try FakeCLIFixture()
      defer { fixture.remove() }
      let sdk = AutohandSDK(configuration: .init(
        cwd: fixture.directory.path,
      ...
      let result = try await sdk.pauseAutoMode()
      #expect(result.success)
      #expect(result.error == nil)

      let request = try #require(requestObjects(in: fixture.requestLog).first {
        $0["method"] as? String == "autohand.automode.pause"
      })
      let parameters = try #require(request["params"] as? [String: Any])
      #expect(parameters.isEmpty)
    }

    @Test func autoModeResumeUsesExactEmptyParametersAndDecodesResult() async throws {
      let fixture = try FakeCLIFixture()
      defer { fixture.remove() }
      let sdk = AutohandSDK(configuration: .init(
        cwd: fixture.directory.path,
      ...
      let result = try await sdk.resumeAutoMode()
      #expect(result.success)
      #expect(result.error == nil)

      let request = try #require(requestObjects(in: fixture.requestLog).first {
        $0["method"] as? String == "autohand.automode.resume"
      })
      let parameters = try #require(request["params"] as? [String: Any])
      #expect(parameters.isEmpty)
    }
      ...
      while IFS= read -r line; do
        ...
        case "$line" in
          ...
          *autohand.session.attach*)
            printf '{"jsonrpc":"2.0","id":%s,"result":{"success":true,"sessionId":"session-existing","workspaceRoot":"/workspace","messageCount":7}}\n' "$id" ;;
          *autohand.yoloSet*|*autohand.yolo.set*)
            printf '{"jsonrpc":"2.0","id":%s,"result":{"success":true,"expiresIn":900}}\n' "$id" ;;
          ...
          *autohand.setContextCompact*)
            printf '{"jsonrpc":"2.0","id":%s,"result":{"enabled":true}}\n' "$id" ;;
          ...
          *autohand.prompt*)
            ...
            printf '{"jsonrpc":"2.0","method":"autohand.automode.complete","params":{"sessionId":"malformed"}}\n'
            ...
            printf '{"jsonrpc":"2.0","method":"autohand.hook.preTool","params":{"toolId":"malformed","malformedMarker":"autohand.hook.preTool"}}\n'
            ...
            printf '{"jsonrpc":"2.0","method":"autohand.hook.postTool","params":{"toolId":"malformed","malformedMarker":"autohand.hook.postTool"}}\n'
            ...
            printf '{"jsonrpc":"2.0","method":"autohand.hook.prePrompt","params":{"instruction":"malformed","malformedMarker":"autohand.hook.prePrompt"}}\n'
            ...
            printf '{"jsonrpc":"2.0","method":"autohand.hook.postResponse","params":{"tokensUsed":42,"tokensUsageStatus":"estimated","toolCallsCount":1,"duration":20,"timestamp":"2026-07-21T00:00:06Z","malformedMarker":"autohand.hook.postResponse"}}\n'
            ...
            printf '{"jsonrpc":"2.0","method":"autohand.hook.fileModified","params":{"filePath":"Sources/Agent.swift","changeType":"renamed","toolId":"tool-1","timestamp":"2026-07-21T00:00:07Z","malformedMarker":"autohand.hook.fileModified"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.fileModified","params":{"filePath":"Sources/Agent.swift","changeType":"modify","toolId":"tool-1","timestamp":"2026-07-21T00:00:07Z"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.sessionError","params":{"error":42,"timestamp":"2026-07-21T00:00:08Z","malformedMarker":"autohand.hook.sessionError"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.sessionError","params":{"error":"Rate limited","code":"RATE_LIMIT","context":{"retryAfter":60},"timestamp":"2026-07-21T00:00:08Z"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.stop","params":{"tokensUsed":700,"tokensUsageStatus":"estimated","toolCallsCount":3,"duration":300.5,"timestamp":"2026-07-21T00:00:09Z","malformedMarker":"autohand.hook.stop"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.stop","params":{"tokensUsed":700,"tokensUsageStatus":"unavailable","toolCallsCount":3,"duration":300.5,"timestamp":"2026-07-21T00:00:09Z"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.sessionStart","params":{"sessionType":"restart","timestamp":"2026-07-21T00:00:10Z","malformedMarker":"autohand.hook.sessionStart"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.sessionStart","params":{"sessionType":"resume","timestamp":"2026-07-21T00:00:10Z"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.sessionEnd","params":{"reason":"timeout","duration":450.5,"timestamp":"2026-07-21T00:00:11Z","malformedMarker":"autohand.hook.sessionEnd"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.sessionEnd","params":{"reason":"clear","duration":450.5,"timestamp":"2026-07-21T00:00:11Z"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.subagentStop","params":{"subagentId":"sub-1","subagentName":"reviewer","subagentType":"code-review","success":"yes","duration":75.5,"timestamp":"2026-07-21T00:00:12Z","malformedMarker":"autohand.hook.subagentStop"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.subagentStop","params":{"subagentId":"sub-1","subagentName":"reviewer","subagentType":"code-review","success":false,"duration":75.5,"error":"Review failed","timestamp":"2026-07-21T00:00:12Z"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.permissionRequest","params":{"tool":"write_file","args":["README.md"],"timestamp":"2026-07-21T00:00:13Z","malformedMarker":"autohand.hook.permissionRequest"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.permissionRequest","params":{"tool":"write_file","path":"README.md","command":"write README.md","args":{"content":"updated"},"timestamp":"2026-07-21T00:00:13Z"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.notification","params":{"notificationType":"warning","message":42,"timestamp":"2026-07-21T00:00:14Z","malformedMarker":"autohand.hook.notification"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.notification","params":{"notificationType":"warning","message":"Context is nearly full","timestamp":"2026-07-21T00:00:14Z"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.contextCompacted","params":{"croppedCount":-1,"usagePercent":0.6125,"reason":"threshold","timestamp":"2026-07-21T00:00:15Z","malformedMarker":"autohand.hook.contextCompacted"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.contextCompacted","params":{"croppedCount":4,"summary":"Earlier turns summarized","usagePercent":0.6125,"reason":"threshold","timestamp":"2026-07-21T00:00:15Z"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.contextOverflow","params":{"tokensBefore":-1,"tokensAfter":80000,"croppedCount":6,"usagePercent":1.05,"timestamp":"2026-07-21T00:00:16Z","malformedMarker":"autohand.hook.contextOverflow"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.contextOverflow","params":{"tokensBefore":120000,"tokensAfter":80000,"croppedCount":6,"usagePercent":1.05,"timestamp":"2026-07-21T00:00:16Z"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.contextWarning","params":{"usagePercent":0.805,"remainingTokens":-1,"timestamp":"2026-07-21T00:00:17Z","malformedMarker":"autohand.hook.contextWarning"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.contextWarning","params":{"usagePercent":0.805,"remainingTokens":12000,"timestamp":"2026-07-21T00:00:17Z"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.contextCritical","params":{"usagePercent":-0.01,"remainingTokens":3000,"timestamp":"2026-07-21T00:00:18Z","malformedMarker":"autohand.hook.contextCritical"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.contextCritical","params":{"usagePercent":0.9575,"remainingTokens":3000,"timestamp":"2026-07-21T00:00:18Z"}}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.mcp.invokeRequest","params":{"requestId":"malformed"}}\n'
            ...
            printf '{"jsonrpc":"2.0","method":"autohand.hook.preTool","params":[1,"known-array"]}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.stop","params":null}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.hook.notification","params":"known-scalar"}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.future.array","params":[2,"future-array"]}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.future.null","params":null}\n'
            printf '{"jsonrpc":"2.0","method":"autohand.future.scalar","params":42}\n'
